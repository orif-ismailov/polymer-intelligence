...
- [ ] **W2 — логотип, frontend — ПРОПУЩЕН по решению владельца** («логотип оставь мне»,
  27.07.2026). Backend W1 закончен и покрыт тестами, так что кусок самодостаточный:
  блок логотипа в `company-view`, аватар в `CompanySwitcher` + directory-picker,
  read-only логотип в карточке компании дашборда.
- [x] **W3** — фото офферов · `0fb861d` (backend) + `64ba060` (UI)
  - T3.1: лимит 8 фото (`MAX_OFFER_IMAGES`, считаются только `kind=image` — TDS не
    занимает слот) → 422 `too_many_files`; `kind=image` обязан быть картинкой
    (`IMAGE_MIMES`) — **тип определяется ДО записи в S3**: первая версия загружала,
    а потом откатывала строку, оставляя осиротевший объект на каждый отказ;
    `DELETE …/files/{id}` возвращает обновлённый оффер (галерея и новая обложка
    из одного ответа); `GET …/files/{id}` — байты владельцу при ЛЮБОМ статусе
    (публичный каталожный эндпоинт отдаёт только approved, т.е. свой черновик
    продавец бы не увидел); `requeue_for_photo_change` — правка фото у
    approved/rejected возвращает в модерацию (зеркало `update_company_offer`);
    `files` + `cover_file_id` в payload через свойства `SellerOffer.photos`/
    `cover_file_id`; у relationship `files` появился явный `order_by=id`.
  - T3.2/T3.3: секция «Фото товара» в форме оффера (до 8, превью, удаление, бейдж
    «Обложка», счётчик n/8, предупреждение о повторной модерации), клиентский
    даунскейл до 1600 px; превью — **Blob через авторизованный эндпоинт** (токен
    в памяти, `<img src>` ушёл бы без заголовка); обложка в списке маркета +
    нейтральный плейсхолдер; галерея (кадр + полоса миниатюр) на странице оффера.
    Создание оффера теперь ведёт на его же экран редактирования — фото цепляются
    к id, до сохранения цеплять не к чему.
  - Обложка выводится на клиенте (`offerPhotos()`), а НЕ добавлена в каталожный
    payload: форма карточки каталога зафиксирована contract-тестом на byte-parity
    с Mini App, новое поле сломало бы пин без выгоды.
  - 14 backend-тестов (сначала красные) + ручная браузерная проверка всего флоу.

### Не сделано в W3 (осознанно)
- **e2e-спека на фото-флоу.** Нужна *верифицированная* компания, а в e2e репозитория
  пути к ней нет: существующий тест «register a company and publish an offer» на самом
  деле заканчивается на экране статуса верификации. Хелпер (авто-approve через
  app-setting + ожидание асинхронных чеков) — отдельная работа по тест-инфраструктуре,
  крупнее самой фичи. Правила покрыты backend-тестами.
- **Фото в карточке модерации дашборда** (T3.3, часть про dashboard) — staff-инструмент,
  не блокирует клиентский DoD.
