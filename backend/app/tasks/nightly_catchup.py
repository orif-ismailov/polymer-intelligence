...
def nightly_llm_catchup() -> dict[str, Any]:
    ...
    from telegram_channel, llm_page and rss sources. Resets each item's
    parse_status to 'pending' and dispatches parse_telegram_item for each.
        ...
        rows = session.execute(
            sa.text(
                ...
                WHERE ri.parse_status = 'budget_deferred'
                  AND s.adapter IN ('telegram_channel', 'llm_page', 'rss')
