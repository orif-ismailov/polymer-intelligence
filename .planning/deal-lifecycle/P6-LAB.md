...
## Progress
- [x] детализация плана → задачи T1.1…T6.2 (`2c979f4`, вместе с W1)
- [x] W1 T1.1 — миграция 0028 + модели + enum'ы (`2c979f4`)
- [x] W2 T2.1–T2.4 — lab_service, матрица, portal + admin API (`b5afeca`)
- [x] W3 T3.1–T3.3 — sample_service, portal API, поля образцов (`a953311`)
- [x] W4 T4.1–T4.2 — бейджи, фильтры, включение веса P4 (`eb6ceef`)
- [x] W5 T5.1–T5.2 — portal + dashboard (`8c541ce`)
- [x] W6 T6.1–T6.2 — staff-карточка Telegram + документация

### Отклонения от плана (сверх пяти поправок к скелету)

1. **`ModerationOfferOut` += `has_lab_passport`/`lab_verified`** — не было ни в скелете,
   ни в детализации. Нашлось в браузере: загрузка паспорта возвращает оффер в модерацию
   «чтобы стафф увидел заявление раньше покупателей», а карточка модерации его не
   показывала — то есть ре-модерация не давала ничего. Исправлено в W5 (`8c541ce`) с тестом.
2. **Portal-API образцов — одна дверь `/transition`, а не пять глаголов** (план обещал
   `accept|decline|sent|received|reject`). Пять маршрутов — пять мест, где можно забыть
   проверку роли; машина и так таблица, а `available_transitions` говорит каждой стороне,
   что ей можно.
3. **Уведомления-колокол живут в W2/W3 рядом с переходами** (записываются в той же
   транзакции), в W6 осталась только Telegram-карточка — как и объявлено в поправке 4.
4. **`lab_partners` читает и аналитик** (план говорил «admin»): выбор лаборатории при
   назначении заявки — часть работы аналитика; на запись справочник остался admin-only.
5. **Тест-фикстура `test_dual_origin_offers._offer`** теперь явно проставляет
   `samples_available`/`lab_verified`: у detached-объекта нет server default'ов, а
   сериализатор типизирует их как `bool` и падает на `None`.

### Что проверено в браузере (Playwright MCP, живой стек)

Демо-скрипт целиком: условия образцов в форме оффера → заявка на анализ («мы свяжемся…»)
→ карточка в очереди дашборда со всеми четырьмя переходами → «Готово» открывает загрузку
PDF → `lab_verified` + «Laboratory Verified» на карточке модерации и в маркете → два
фильтра → запрос образца покупателем → приёмка и отправка продавцом (курьер + трек) →
`Получено` покупателем. Два входа в портал (продавец и покупатель), полный audit-след.

### Заведомые пробелы (подтверждены по факту)

- **e2e-спеки для P6 нет** — в репозитории по-прежнему нет e2e-пути к верифицированной
  компании (пробел P1/P2/P3/P5). Проверка ручная, браузерная; портальные 23 e2e зелёные.
- **SLA лаборатории не моделируется** — процесс ручной, сроки вне контроля платформы.
- **AI-совместимость по лаб-параметрам** (MFI/плотность → «подходит ли под литьё») —
  опция R5+ из ТЗ §3.3, требует структурированного разбора паспорта; в P6 не входит.
- **Паспорт одобренного оффера отдаётся публичным маршрутом** `/webapp/market/offers/{id}/images/{file_id}`
  (поведение Phase 2, распространяется на все файлы одобренного оффера). P6 его не расширяет,
  но и не сужает — если публичность SDS/COA/паспорта окажется нежелательной, это отдельная
  задача по всему файловому маршруту, а не по лаборатории.
