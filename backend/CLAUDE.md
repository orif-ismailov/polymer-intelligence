...
- **Prompts are immutable + versioned** across five families (`extract`, `news_extract`, `report`,
  `analyze_request`, `substance_match`). The rule protects exactly one property — **a version string
  means one text, forever** — because `parse_runs.prompt_version` is the only record of what
  produced a past classification, and each loader caches per process keyed on that string. Editing
  a version in place would leave some workers on the old text and some on the new, both journalling
  the same version, and nothing could tell them apart afterwards.
  - For four families a change is a new `parsing/prompts/<family>_v{N+1}.md` + a bump of its pin
    (`substance_match` pins in `substance_ai_service.PROMPT_VERSION`; `extract`/`report`/
    `analyze_request` pin via env — `LLM_PROMPT_VERSION`, `REPORT_PROMPT_VERSION`,
    `REQUEST_AI_ANALYSIS_PROMPT_VERSION`).
  - **`news_extract` can also be authored from the dashboard** (`/admin/settings/news`). Saving
    writes a NEW row in `prompt_versions` (append-only, migration `0046`, no `updated_at` and no
    update path); activating one is an ordinary `news_prompt_version` override, so it inherits that
    rail's validation, audit row and cross-process propagation. Shipped files stay the fallback:
    `resolve = authored row, else news_extract_<version>.md`, and `allowed_values` is the union.
  - **`load_news_prompt` must never read the database.** It runs inside `parse_news_item`'s open
    transaction, so a checkout there is the nested-connection deadlock `settings_service.get()` is
    forbidden from causing — authored bodies ride in the settings snapshot, loaded by `refresh(db)`
    on a session the caller already owns. The `lru_cache` covers only the shipped file read, which
    cannot change under a running process.
  - **No loader may answer `""` for a missing version.** An empty string is a *valid* system prompt,
    so the classifier runs with no instructions and the only symptom is bad classification. Fixed in
    `news_extract` and `extract`; **`app/domains/news/reports.py::_load_prompt` still does it** and
    masks it with `or _load_prompt("v1")`.
  - **The news family has no eval harness.** `parsing/eval_cli.py` and the frozen
    `tests/parsing/golden/predictions/extract_v1.json` are `extract`-only. The dry-run button
    (`POST /admin/settings/news-prompt/try`) is the only check between an authored prompt and every
    article classified after it — it spends real tokens and settles them against the daily budget,
    and it deliberately writes no `parse_run`, because a journal row for a prompt that was never
    active is the ambiguity this design exists to prevent.
- **Analytics** (`app/services/analytics_service.py`) is read-only over data that already existed.
  Three things to know before touching it:
  - **`_month_bounds` is Tashkent's month**, and the Didox figures depend on it. A call at 20:00
    UTC on the 31st is already the 1st for the operator; counted in UTC it lands in the wrong
    month and both totals still look reasonable.
  - **`NOT_SENT = "breaker_open"` is a contract with a string literal in
    `app/integrations/didox/client.py`.** Those rows are written before any request leaves the
    process, so they consume no quota. `tests/test_analytics.py` asserts the literal still matches;
    if it drifts, consumption silently gains every call the breaker refused.
  - **`_AI_SOURCES` must list every LLM caller.** Spend that is not counted is worse than spend
    that is not shown, because the page prints a total. The guard is
    `TestEveryAiFeatureIsCounted`, which compares it against `llm_clients._CLIENTS`.
  - The SQL lives in `tests/test_analytics_db.py` (real Postgres). `FILTER`, `percentile_cont`,
    `AT TIME ZONE` and a five-arm `UNION ALL` cannot be checked against a stub — two of these
    queries shipped wrong (`verification_cases.updated_at`, which does not exist) and only the
    database said so.
- **Runtime feature switches** (`app/services/settings_service.py`) resolve **override row, else
  `.env`** — ~30 of them: news/report toggles, `escrow_mode`, the `rfq_supplier_push_*` set,
  **P5** `dangerous_check_enforced` (publish gate, ships OFF), `substance_ai_enabled`,
  `chem_registry_mode`, **P7.c** `gov_registry_mode` (ships `stub`; `didox` reads the tax registry
  through the P7.a gateway; `live` raises until a ПЦД adapter exists), **P7.a** `didox_mode`
  (the DOCUMENT rail — deliberately separate, because reading the registry is harmless and sending
  legally significant documents is not) and the four `didox_*` connection settings, plus the
  Telegram notify routing and the `ingest_http_*` tunables.
  - `.env` is the DEFAULT and the complete contract; `app_settings` (migration `0045`) stores only
    overrides, written from `/admin/settings`. This does not undo `0043`: what that removed was a
    table storing a value beside a default written as a Python literal here, so a fresh DB ran every
    rail on something invisible (the Didox 503, 31.08.2026, a day to find). The defaults live on
    `Settings` now and stay there — only the override layer came back, and the panel shows both
    values with the env var named and a Reset button, so an override reads AS an override.
  - **`get(key)` takes no `db` and must never acquire one.** It runs inside open transactions
    (`verification/service.py` holds a `SELECT … FOR UPDATE` across it); with `DB_POOL_SIZE=5`
    against uvicorn's 40-thread pool a second checkout per call is a pool deadlock. Overrides arrive
    via `refresh(db)` from `get_db()` and Celery `task_prerun`, generation-gated through Redis
    (0.25 s timeout, compared with `!=` so a `FLUSHALL` resets cleanly rather than going stale
    forever). Postgres is authoritative; Redis only signals. `get_int` / `get_float` / `get_str` /
    `get_bool` narrow the `SettingValue` union at the call site.
  - **A new spec is `overridable=False` by default** — env-only until somebody opts it in, which is
    a one-line diff a reviewer can see. `sensitive=True` adds Fernet-at-rest, masking, and an
    `is_admin` requirement on top of the page grant.
  - **The `ai` group is operator-settable** (`/admin/settings/ai`): both LLM keys and both model
    pins. Four specs, and the only ones with hooks around the write:
    - A key is **verified with its provider before it is stored** — `llm_clients.verify()` reads
      `models.list()`, which costs no tokens — and a key the provider refuses is rejected with the
      provider's own reason. The trade is deliberate: a rotation is blocked while the provider is
      unreachable, but every LLM feature here DEGRADES rather than errors on a bad key (news
      classifies badly, the report falls back to rule-based), so a stored typo would be expensive
      and silent. The key in use keeps working throughout. **The two vendors nest that reason
      differently** — Anthropic's `exc.body` is the envelope (`{"error": {"message": …}}`),
      OpenAI's is already the inner error (`{"message": …}`) — and `_reason` checks both; written
      for one shape only, the banner falls back to the SDK's Python repr of the whole envelope.
    - **Five modules hold clients built at import** (`parsing.extractor`,
      `parsing.news_extractor`, `app.domains.news.reports`, `app.domains.requests.analysis`,
      `app.domains.compliance.substance_ai`) — `_client` for Anthropic and `_openai_client` for
      OpenAI — so `_invalidate` fires `llm_clients.reset()`, which **rebinds both in place** in
      whichever of them `sys.modules` already has. Rebinding, not a lazy factory: `_client` staying
      a module attribute is what keeps the ten `patch("parsing.extractor._client")` sites in the
      suite working. `reports` is the odd one out — RAW clients, hence the `wrapped` flag in
      `_CLIENTS`. `reset()` never raises; a rebind that failed must not block the operator fixing
      the key.
    - **The OpenAI client is built at IMPORT as well as on reset**, and is `None` without a key.
      Both halves matter: `reset()` fires only on a CHANGE, so a process that boots with a good
      `OPENAI_API_KEY` and is never reconfigured would otherwise hold `None` and refuse every GPT
      call — and `openai.OpenAI(api_key="")` RAISES at construction, so an unguarded build would
      make `reset()` fail in every process on every deployment that has no OpenAI key.
  - **Two vendors, one call path.** `llm_clients.provider_of(model)` derives the vendor from the
    model id; an id matching no known OpenAI prefix goes to Anthropic, so this cannot move an
    existing deployment. `structured()` serves the four extraction sites and `text()` the report
    digest. The traps live in `_request_kwargs` and `usage_of`, are all invisible to a mocked test,
    and are listed in the root `CLAUDE.md`: system-as-message, `max_completion_tokens`, no
    `temperature` on reasoning models, and OpenAI's `prompt_tokens` INCLUDING cached tokens.
    `cache_control` stays Anthropic-only on purpose.
  - **Every read site of an overridable key must go through `settings_service`.** Reading
    `settings.X` directly gives the env value and ignores the panel — a control that silently does
    nothing. `test_settings_env_source.py` guards the notification accessors.
  - Adding a switch is a field on `Settings`, a `SettingSpec` naming it, then **`make env-sync`** —
    which appends the `deploy/.env.example` line with the real default and the right `[panel: X]`
    marker, leaving one sentence of prose to write. `tests/test_settings_env_source.py` fails if a
    spec points at a field that does not exist; `tests/test_env_contract_sync.py` fails if the
    contract drifts, a marker is stale, or a documented value is one `Settings` would refuse.
  - **`scripts/sync_env_example.py` never rewrites a VALUE**, only the markers and missing lines.
    Two documented values differ from their code default on purpose — `S3_ENDPOINT` shows the
    compose address where the field defaults to empty, and `CORS_ALLOWED_ORIGINS` shows the
    comma-separated env form a `mode="before"` validator splits into a list — so a tool that forced
    value == default would corrupt both. The value guarantee is weaker and true: every documented
    value is one `Settings` would accept.
  - **In tests, flip a switch with `set_switch(...)` from `conftest`**, which writes the OVERRIDE
    layer so the test exercises the real precedence path. An autouse fixture clears the snapshot
    after each test and pins `AUTO_REFRESH = False` for the suite (there is no Redis in CI).
    Real-DB coverage of the round trip is `tests/test_settings_overrides_db.py`.
  - `Settings` reads **one** env file: the repo-root `.env`, by absolute path. There is no
    `backend/.env` any more (merged in and deleted — the two disagreed on ten keys, and which one
    won depended on your working directory). Anything CWD-relative here is a bug, and
    `test_settings_env_source.py` fails if a `backend/.env` reappears.
