...
import { useLabPool } from "@/entities/lab-request";
import type { LabPoolItem } from "@/entities/lab-request";
import { LabChat } from "@/features/lab-chat";
import { labRequestApi } from "@/entities/lab-request";
...
/**
 * `/cabinet/requests` for a company whose confirmed role is laboratory.
 *
 * A lab files no polymer purchase requests, so «Заявки» was permanently empty
 * for it — this is what belongs in that slot: every open analysis request a
 * buyer has broadcast, and the conversations this lab has already started.
 *
 * The buyer's Заявки page is untouched; `RequestsRouteSwitch` picks between the
 * three. Deliberately NOT the P6 `/cabinet/lab-orders` queue, which is the
 * staff-run partner-lab flow and belongs to a different audience.
 */
export function LabPoolPage() {
  const { t, i18n } = useTranslation();
  const lang = coerceLang(i18n.language);
  const [tab, setTab] = useState<"all" | "mine">("all");
  const { activeCompany, isLoading } = useActiveCompany();
  const companyId = activeCompany?.id ?? null;
  const query = useLabPool(companyId);
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title={t("labRequest.poolTitle")}
        subtitle={t("labRequest.poolSubtitle")}
      />
      ...
      <div className="min-w-0">
        <Tabs
          items={[
            { id: "all", label: t("labRequest.tabAll"), count: items.length },
            {
              id: "mine",
              label: t("labRequest.tabMine"),
              count: items.filter((i) => i.my_thread_id != null).length,
            },
          ]}
          value={tab}
          onChange={(id) => setTab(id as "all" | "mine")}
          label={t("labRequest.poolTitle")}
        />
      </div>
      ...
      {!query.isLoading && shown.length === 0 ? (
        <EmptyState
          ...
          title={t("labRequest.poolEmptyTitle")}
          description={t(
            tab === "mine"
              ? "labRequest.poolEmptyMineBody"
              : "labRequest.poolEmptyBody",
          )}
        />
      ) : null}
...
function PoolCard({ item, companyId, lang, open, onToggle, onOpened }: PoolCardProps) {
  ...
  async function reply(): Promise<void> {
    ...
    try {
      ...
      const thread = await labRequestApi.openThread(item.id, companyId);
    ...
    <li data-request-id={item.id}>
      <Card>
        <CardBody className="space-y-3">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="truncate text-sm font-medium text-text">
                {item.product_text}
                {item.grade_text ? ` · ${item.grade_text}` : ""}
              </p>
              <p className="mt-0.5 text-xs text-text-muted">
                {item.methods
                  .map((m) => t(`labRequest.methodOptions.${m}`, { defaultValue: m }))
                  .join(", ") || "—"}
                {item.sample_qty ? ` · ${item.sample_qty}` : ""}
              </p>
            ...
            <div className="flex shrink-0 items-center gap-2">
              ...
              {threadId == null ? (
                ...
                <Button
                  ...
                  variant="outline"
                  onClick={onToggle}
                  data-testid={`lab-open-${item.id}`}
                >
                  {t("labRequest.openChat")}
                </Button>
              )}
            </div>
          </div>
          ...
          {item.comment ? (
            <p className="text-xs leading-relaxed text-text-muted">{item.comment}</p>
          ) : null}

          {open && threadId != null ? (
            <div className="border-t border-border pt-3">
              <LabChat companyId={companyId} threadId={threadId} />
            </div>
          ) : null}
        </CardBody>
      </Card>
