...
Every other route (the authenticated cabinet behind the login form) is sent the app shell
...
2. **Run the demo on the dev stack** (R1-PLAN Definition of Done): submit the access request at
   `/cabinet/register` → issue credentials for it from the dashboard's «Кабинеты клиентов»
   (admin-only; the password is shown once) → sign in with them and set a new password at the
   forced-change screen → create 2 companies → submit → approve one from dashboard
   `/verification`, the other from the Telegram group → switch active company → publish an offer →
   moderate → offer appears in the public market with `company_verified: true`.

   For a demo that skips the issuing step, the showcase seeder gives every demo person
   `<company key>-<role>` (e.g. `shurtan-owner`) with `SEED_DEMO_PASSWORD`.
