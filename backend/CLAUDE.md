...
| `app/integrations/` | External gateway adapters: `sms/` (console/eskiz) + **R3** `eimzo/` (UNICON e-imzo-server sidecar client — `verify_pkcs7`, `EIMZO_STUB` dev mode) + `circuit_breaker.py` (reusable, gateway-wide). |
...
| `alembic/versions/` | Migration chain `0001`→`0021` (`0017` R1 verification/portal, `0018` R2 dual-origin requests/inquiries + `portal_notifications`, `0019` market-list index, `0020` R3 E-IMZO rails — `signature_evidence`/`company_person_data`/`integration_call_log`, `0021` R3 contracts). |
